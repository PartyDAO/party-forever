...
export const PartyBidSearch = () => {
  const navigate = useNavigate();
  const { register, handleSubmit } = useForm<FormData>();

  const onSubmit = (data: FormData) => {
    navigate(`/party/${data.contractAddress.trim()}`);
  };

  return (
    <Card className="w-full max-w-md">
      <CardHeader>
        <CardTitle>PartyBid</CardTitle>
        <CardDescription>Enter a PartyBid address to view details</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="partybid-address">Contract Address</Label>
            <Input id="partybid-address" placeholder="0x..." {...register("contractAddress")} />
          </div>
